...
def extract_blocks_from_sheet(ws):
    ...
    print(f"[DEBUG] Tab '{ws.title}': encontre {len(first_rows)} etiqueta(s) 'FIRST' "
          f"en las filas: {first_rows}")

    # Diagnostico extra: busca CUALQUIER celda que contenga "first" como
    # substring (no exacto), para detectar variantes como "First *",
    # "First:", "First Name", espacios raros, etc.
    print("[DEBUG] Buscando variantes de 'first' en toda la hoja...")
    for row in ws.iter_rows():
        for cell in row:
            if cell.value is None:
                continue
            ...
            if "first" in text.lower():
                print(f"[DEBUG]   Fila {cell.row}, Columna {cell.column}: {text!r}")
