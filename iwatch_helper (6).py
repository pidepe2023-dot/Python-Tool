...
# Pausa (en segundos) despues de pegar y presionar Tab en cada campo, antes
# de pasar al siguiente. Si notas que el auto-llenado va muy rapido y se
# salta o mezcla campos (sobre todo justo despues de elegir Country/State),
# sube este numero (ej. 0.6 o 0.8). Si va muy lento, puedes bajarlo un poco.
FIELD_DELAY_SECONDS = 0.4
...
def _extract_fields_from_col(ws, row_start, row_end, label_col, value_col):
    """
    Como _extract_fields_from_rows, pero SOLO mira una columna de etiquetas
    (label_col). El valor normalmente esta en value_col (la columna
    inmediata a la derecha), pero por celdas fusionadas con offset
    (ej. "INTERNATIONAL CODE" cuyo valor cae 2 columnas mas alla en vez de
    justo al lado) se revisan hasta 3 columnas a la derecha por si acaso.
    Esto es necesario cuando hay dos tablas lado a lado en las mismas filas
    (ej. Owner en columnas B:C y Compliance Officer en columnas F:G).
    """
    found = {}
    for r in range(row_start, row_end + 1):
        ...
        for field in FIELD_MAP:
            ...
            for kw in field["keywords"]:
                if kw == text or text.startswith(kw):
                    # Busca el primer valor no vacio entre value_col y value_col+2
...
def extract_blocks_from_sheet(ws):
    ...
    COMPLIANCE OFFICER, etc.), apilados verticalmente O uno al lado del
    otro en las mismas filas pero en columnas distintas. Esta funcion:
      1. Encuentra CADA celda donde aparece la etiqueta "FIRST" (fila Y
         columna) -> ahi empieza un bloque nuevo, en esa columna.
      2. Para cada uno, calcula hasta donde llega su tabla bajando por esa
         MISMA columna hasta encontrar 2 filas vacias seguidas (o el
         siguiente "FIRST" en esa misma columna, o un limite de 30 filas).
      3. Busca hacia arriba, cerca de esa columna, el titulo del bloque
         (texto en mayusculas que no sea una etiqueta de campo conocida).
    ...
    print(f"[DEBUG] Tab '{ws.title}': encontre {len(anchors)} etiqueta(s) 'FIRST' en: {anchors}")
    ...
    for i, (start_row, label_col) in enumerate(anchors):
        ...
        # Limite: no cruzar hacia el siguiente "FIRST" que este en la MISMA columna
        ...
        # Baja por la columna hasta 2 filas vacias seguidas (fin de tabla)
        end_row = start_row
        empty_streak = 0
        r = start_row
        while r <= hard_limit:
        ...
        # Busca el titulo del bloque: sube hasta 8 filas, mirando columnas
        # cercanas (por si el titulo esta centrado/fusionado un poco distinto)
    ...
    print(f"[DEBUG] Bloques detectados: {[b['name'] for b in blocks]}")
    return blocks
    ...
    def _run_autofill(self):
        ...
        for label, value in self.fields:
            if label in DROPDOWN_FIELDS:
                ...
                # Pausa extra despues de cerrar el mensaje: le da tiempo a
                # Windows de terminar de minimizar la ventana y a iWatch de
                # quedar con el foco de vuelta, ANTES de mover el Tab.
                time.sleep(FIELD_DELAY_SECONDS * 2)
                pyautogui.press("tab")
                # Pausa extra despues del Tab: el campo siguiente (ej. City)
                # necesita un momento para quedar realmente activo antes de
                # que le peguemos el valor, si no, se puede saltar.
                time.sleep(FIELD_DELAY_SECONDS * 2)
                continue
